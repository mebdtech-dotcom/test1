...
| P-ACC-19 | Lead credits | P2 | Ready | 🟡 In Progress | Building — T-LISTING at /account/lead-credits; frozen `get_lead_balance`/`list_lead_transactions`/`credit_lead_account` (BC-BILL-4); balance stat-card (NUMERIC credits, Inv #10) + transactions table (date·type·delta·balance, tabular-nums, **deltas +/- sign not colour-only**); type filter; pagination; `credit_lead_account` admin-gated (Doc-3 §6 shortfall credit); platform lead packages = platform revenue (no buyer↔vendor money) |

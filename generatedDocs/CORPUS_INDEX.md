...
**Carried into Doc-6F…6K (per-module gates):** `DR-6-CORE` (M0 owns outbox/audit/id/config) · `DR-6-API` (Doc-5 persistability cross-check) · `DR-6-STATE` (state machines as columns + enforcement) · `[ESC-6-SCHEMA]` (additive Doc-2 patch) · `[ESC-6-POLICY]` (additive Doc-3 §12.2 patch) · `[ESC-6-API]` (Board → additive Doc-2 patch). **Doc-6B (M0 `core`) + Doc-6C (M1 `identity`) + Doc-6D (M2 `marketplace`) + Doc-6E (M3 `rfq`) FROZEN 2026-06-26.** Doc-6E carries `[ESC-RFQ-AUDIT]` · `[ESC-RFQ-SCHEMA-RULES]` — all on named channels. **Next:** Doc-6F (M4 `operations`) — post-award ops + the private Vendor CRM (`buyer_vendor_statuses` — the blacklist's owning side, non-disclosure invariant) + two-sided engagement + the money-record boundary.

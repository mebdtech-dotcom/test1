...
| `Doc-8B_Structure_v1.0_FROZEN.md` (Test Foundation & Harness — structure) | STRUCTURE FROZEN | v1.0 | ✅ | First Doc-8 realization; the shared harness Doc-8C…8G consume (frozen first per DR-8-HARNESS). Realizes Doc-8A §4/§10 + Appendix A bands A/H/I: runner, ephemeral test DB, fixtures/factories (through-contracts), ≥2-org seeding, seeded clock + deterministic UUIDv7 provider, out-of-wire mock boundary + **outbox observer/drainer** (Band-F enabler), CI merge-gate. **D1 Board-ratified → `[ESC-8-TOOLING]` RESOLVED: Vitest + Playwright + TS-native transactional SQL** (single TS toolchain; pgTAP not selected). Effective = `Doc-8B_Structure_Proposal_v0.1` + `Doc-8B_Structure_Patch_v0.1.1`. **Content passes NOT STARTED.** Coins nothing |
| `Doc-8B_Structure_Proposal_v0.1.md` · `Doc-8B_Structure_Independent_Hard_Review_v0.1.md` · `Doc-8B_Structure_Patch_v0.1.1.md` · `Doc-8B_Structure_Freeze_Audit_v1.0.md` | PROVENANCE | v1.0 | — | Doc-8B structure authoring history (Pass → Board Hard Review [2 MINOR + 1 OBS + 1 NIT; 1 REJECTED] → Structure Patch + short re-review PASS → Structure Freeze Audit PASS; D1 tooling Board-ratified) |

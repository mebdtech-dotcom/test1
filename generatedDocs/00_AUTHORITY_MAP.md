...
| `Doc-5G_Structure_v1.0_FROZEN.md` (M5 `trust` API realization) | STRUCTURE FROZEN | v1.0 | ✅ | Canonical TOC realizing Doc-4G (40 contracts: 34 caller-facing + 6 out-of-wire) on HTTP; the governance-signal owner; multi-actor (Public/User/Admin); R1–R12 (score-computation + governance/Billing firewalls); DG-1…DG-8; SR-1 reconciled (40; `decide_verification`). Content passes next (3). `[ESC-TRUST-POLICY]` wire-referenced keys + `reference_id` = content-freeze obligations |
| `Doc-5G_Structure_Proposal_v0.1.md` | PROVENANCE | v0.1+r1 | — | Doc-5G structure authoring history (Hard Review — SR-1 reconciliation + ADD-1/ADD-2) |

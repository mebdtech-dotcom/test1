...
| **Renumber note** | **Two renumbers.** (1) *On branch:* authored on `fe/account-referral-nav` as v1.0.9 / PATCH-D2-08, then renumbered to **v1.0.10 / PATCH-D2-09** (2026-07-19) when the parallel governance train folded the **Vendor-Buyer-Relationship** patch into v1.0.9 / PATCH-D2-08 (`573a349`), consuming both the number and the patch ID **and** the §7 count transition (tenant 36→37, total 45→46 — this patch's baseline). (2) *Reconcile to `main`* (2026-07-23, D2-09 forward-PR): renumbered to **v1.0.11 / PATCH-D2-10** per `governanceReviews/RULING_Doc-2_v1.0.9_Collision_Renumber_SchemeB_v1.0.md` (Scheme B) — on `main`, v1.0.9 is the **Communication Audit** patch (PATCH-D2-09) and VBR folded as `Doc-2_Patch_v1.0.10_VendorBuyerRelationship.md` (PATCH-D2-08), so both axes move up one. **Identifier reassignment only; content unchanged.** |
| **Date** | 2026-07-19 |

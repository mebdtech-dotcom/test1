...
| Status | **PROPOSAL v0.1** — pre-freeze; open for Board Review |
...
- **R7 — Subscription state machine authority (Doc-2 §5.7; Doc-4M = cross-module state-map index).** BC-BILL-2 is the **sole subscription lifecycle authority** — `pending_payment → active → expired` (purchase → `pending_payment → active`; period-end renew → `active`; cancel sets `auto_renew=false`, runs to period end; period-end expire → `expired`; expired repurchase → `pending_payment`). Plans machine: `draft → active → retired` (BC-BILL-1, `Doc-2 §3.8`). **No edge added or modified.** Illegal transition → `STATE` → `409`; lost race → `CONFLICT` → `409`. (Doc-4M = index only; `Doc-4A §13`.)
...
- **R9 — M7 emits exactly 3 Doc-2 §8 events (BC-BILL-2 only).** `SubscriptionPurchased` · `SubscriptionRenewed` · `SubscriptionExpired` — single-authorship, BC-BILL-2 producer; consumed by Communication for fan-out (DF-BILL-6). BC-BILL-1/3/4/5/6 emit **no** Doc-2 §8 event. Lead-access and advertising/microsite metering signals are carried as `[ESC-BILL-EVENT]` (no §8 emission event today). `QuotationSubmitted` is RFQ-owned / Billing-consumed (DF-BILL-3). No event coined.
...
| BC-BILL-3: `enforce_quota` | Internal entitlement-bounded quota check (21.3 System; R10) | **§10** out-of-wire |
| BC-BILL-2: `resolve_entitlements` | Entitlement-resolution authority (21.3 User/System; consumed cross-module via service call; R10) | **§10** out-of-wire |
...
| **DF-BILL-5** | Operations — `billing.platform_invoices ≠ operations.trade_invoices` (FIXED; DF-6 in Doc-5F); Operations never accesses Billing tables; Billing never accesses Operations trade invoices | Enforced as R6 fence + §8 wire constraint; disjoint ownership stated; no Operations surface realized | **No** |

...
| **DD-6** | `marketplace.*` **POLICY-key** registration (dedup window, page/assignment/placement caps, domain-verify window) | Windows/caps referenced by **intended key name** by pointer; registration via the **Doc-3 §12.2 additive channel** — an additive `marketplace.*` registration patch (**precedent: `Doc-3_Policy_Key_Registration_Patch_v1.0` (`core.*`), `…v1.1_RFQ` (`rfq.*`)**), human/Board-approved, satisfying **Doc-4A §18.2**; **`[DD-6]`-keyed contracts not finalized until registered**. Not invented here (`CHK-5A-121/123`). | **Structure: No.** **Content: YES — CHK-5A-121 content-freeze gate** (per the Doc-5E `[ESC-RFQ-POLICY]` precedent): the content Freeze Audit **blocks** if any referenced `marketplace.*` key is absent from Doc-3 §12.2. Resolve via the registration patch first. |
| **DD-7** | `vendor_claim_records` tenancy-class ambiguity (Doc-2 §6 platform-owned vs §10.3/§3.3 marketplace child) — pre-existing Doc-2 tension | **⚠ Cross-frozen-doc conflict — FLAG-AND-HALT, Board-gated (parallels Doc-5C O-01 / Doc-4M delegation):** two rank-0 Doc-2 sections diverge on `vendor_claim_records` ownership. `claim_vendor_profile` mutates it; **mutation ownership is NOT resolved here** (architecture-touching → human/Board; never a Doc-5D decision). Resolution channel: an additive Doc-2 §6/§3.3 reconciliation. **DD-7 blocks `claim_vendor_profile` content finalization only — it does NOT block Doc-5D structure freeze.** | **Tracked / escalated** — content-finalization blocker; **does not gate structure freeze** |
...
| **ADD-1** (round-3, post-Doc-5E freeze) DD-6 POLICY-key posture stale vs the Doc-5E `[ESC-RFQ-POLICY]` precedent | MAJOR | **FIXED** — DD-6 now foreshadows the additive Doc-3 §12.2 `marketplace.*` registration patch (precedent `core.*` v1.0 / `rfq.*` v1.1; Doc-4A §18.2) and flags it a **CHK-5A-121 content-freeze gate** (non-gate for structure freeze). |
| **ADD-2** (round-3) DD-7 cross-frozen-doc tension under-labeled | MINOR | **FIXED** — DD-7 now tagged a flag-and-halt, Board-gated cross-frozen-doc conflict (parallels Doc-5C O-01); no ownership decision here; content-finalization blocker only. |

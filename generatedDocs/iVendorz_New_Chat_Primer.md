...
1. **Doc-6J (M8 `admin`):** next per-module schema; gated by Doc-6A Appendix A (`CHK-6-xxx`, 10 bands / 37 checks). Moderation/bans/category-approval/import/verification-tasks/outreach; **the authoritative event catalog (Doc-4J)**; "Admin decides, owning module owns" (ban authority M2 reflects, verification-tasks M5 stores); never-vendor-visible `link_suggestions`. Then Doc-6K (M9 `ai`).

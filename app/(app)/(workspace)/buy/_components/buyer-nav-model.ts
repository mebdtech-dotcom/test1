...
// link. `Draft Requests` / `Closed RFQs` filter the SAME `/buy/rfqs` page via a real frozen Doc-4M state
...
// pointed at `/account/members|organization|profile|settings` and `/buy/notifications` — all OUTSIDE the
...
// pages (`(buyer)/team`, `/buy/organization`, `/buy/profile`, `/buy/settings`) compose the EXISTING, UNMODIFIED
// Account view components (`MembersView`, `OrganizationProfile`, `UserProfileForm`,
// `SecuritySettings`, `NotificationPreferences`) inside the persistent Buyer shell; `/buy/notifications`
...
export const BUYER_NAV: NavSection[] = [
  { id: "overview", items: [{ label: "Dashboard", href: "/buy/dashboard", icon: "dashboard" }] },
  {
    id: "procurement",
    label: "Procurement",
    items: [
      {
        ...
        icon: "rfqs",
        children: [
          { label: "My RFQs", href: "/buy/rfqs" },
          { label: "Draft Requests", href: "/buy/rfqs?state=draft" },
          { label: "Closed RFQs", href: "/buy/rfqs?state=closed" },
          { label: "Approvals", href: "/buy/approvals" },
        ],
      },
      {
        label: "Quotations",
        ...
        icon: "quotations",
        children: [
          { label: "Received Quotes", href: "/buy/quotations" },
          { label: "Compare Quotes", href: "/buy/quotations/compare" },
        ],
      },
      {
        ...
        icon: "orders",
        children: [
          { label: "Active Orders", href: "/buy/purchase-orders?status=active" },
          { label: "Order History", href: "/buy/purchase-orders?status=history" },
          { label: "Engagements", href: "/buy/engagements" },
        ],
      },
      // FE-DOC-01 (P-DOC-01): the cross-workspace Documents hub, buyer leg (page_inventory §12).
      { label: "Documents", href: "/buy/documents", icon: "documents" },
    ],
  },
  {
    id: "marketplace",
    label: "Marketplace",
    items: [
      { label: "Vendor Directory", href: "/buy/discover", icon: "discover" },
      { label: "Saved Vendors", href: "/buy/saved-vendors", icon: "favorites" },
      { label: "Specification Library", href: "/buy/spec-library", icon: "specLibrary" },
      // Buyer-private (Invariant #11) — never leaks a CRM status/flag anywhere else in this nav.
      { label: "Vendor CRM", href: "/buy/crm", icon: "crm" },
    ],
  },
  {
    id: "communication",
    label: "Communication",
    items: [
      { label: "Messages", href: "/buy/messages", icon: "messages" },
      { label: "Notifications", href: "/buy/notifications", icon: "notifications" },
    ],
  },
  {
    id: "analytics",
    label: "Analytics",
    items: [{ label: "Reports & Analytics", href: "/buy/reports", icon: "reports" }],
  },
  {
    id: "organization",
    label: "Organization",
    items: [
      { label: "Team", href: "/buy/team", icon: "team" },
      { label: "Organization", href: "/buy/organization", icon: "org" },
    ],
  },
  {
    id: "account",
    label: "Account",
    items: [
      { label: "Profile", href: "/buy/profile", icon: "account" },
      { label: "Settings", href: "/buy/settings", icon: "settings" },
    ],
  },
];
...
export const BUYER_QUICK_CREATE: QuickCreateItem[] = [
  { label: "New RFQ", href: "/buy/rfqs/new", icon: "rfqs" },
];

/** Mobile quick-bar — a thumb-reach SUBSET of the Buyer nav. */
export const BUYER_QUICK_BAR: NavItem[] = [
  { label: "Dashboard", href: "/buy/dashboard", icon: "dashboard" },
  { label: "RFQs", href: "/buy/rfqs", icon: "rfqs" },
  { label: "Approvals", href: "/buy/approvals", icon: "approvals" },
  { label: "CRM", href: "/buy/crm", icon: "crm" },
];

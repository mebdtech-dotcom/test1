// Notification center — full page (`/buy/notifications`) — P-SH-02 (Doc-7C Shared Authenticated Shell ·
...
// Team-4"). Zero non-buyer code referenced `/buy/notifications` (grepped repo-wide) — buyer was its only
...
// path under `/sell/*` or `/admin/*`, the same pattern every other cross-workspace concept in
// this codebase already follows (e.g. vendor's own `/sell/settings`, `/sell/organization`).
            ...
            {FILTERS.map((f) => {
              ...
              return (
                <li key={f.key}>
                  <a
                    href={
                      f.key === "all" ? "/buy/notifications" : `/buy/notifications?filter=${f.key}`
                    }

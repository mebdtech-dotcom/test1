# Doc-5D_VendorSlugResolve_Additive_Patch_PROPOSAL.md

> **STATUS: PROPOSAL — awaiting owner/Board ruling.** Not yet folded into the corpus. Wire
> realization of `marketplace.resolve_vendor_slug.v1`, the contract proposed in the linked pair
> `Doc-4D_VendorSlugResolve_Additive_Patch_PROPOSAL.md`. Both are proposed to be approved and
> folded **together** — this document defines no contract of its own.
...
capability, geography, or any other profile field on any branch — the caller chains into
`get_public_vendor_profile.v1` for that. No `human_ref` on the `current` branch (the caller only
needs `vendor_profile_id` to make that next call).
...
- **Uniform collapse — the two-hop case is the one that must be tested, not assumed:** absent slug,
  ...
  }` — byte-identical status, body, and timing across all three causes and every cache tier. This
  is the direct realization of the Doc-4D patch's Non-Disclosure clause (Invariant #11) — the
  conformance obligation below (Part 2) exists specifically because this is the least obvious of
  the three response branches to get right.
...
registered by this patch.** Adoption is explicit per that key's own registration text
("adoption is explicit and per-endpoint... no automatic/implicit adoption").
...
| V-2 | **Migrated-slug redirect only to a currently-visible target** — a `vendor_slug_history` match whose target vendor is banned/unpublished/soft-deleted at read time returns `not_found`, never `migrated` (this is the case V-1 exists to name explicitly) |
| V-3 | **`current` branch never returns profile fields** — response shape contains only `vendor_profile_id`, nothing else, on every implementation |
| V-4 | **Case-sensitivity**: a request `slug` containing any uppercase character never matches a stored (always-lowercase) slug — falls through to `not_found`, not a case-insensitive match |
...
per `Doc-6D_VendorSlugSubdomain_Patch_v1.0.1` 6D-VSS-01.3 — this patch adds a read realization
only) · no change to `get_public_vendor_profile.v1`'s own wire realization (row 64, unchanged) ·
no matching/ranking/discovery-ordering surface created.

# Downstream resolution — proposed to land together at this fold

- **`Doc-4D_VendorSlugResolve_Additive_Patch_PROPOSAL.md` (linked pair):** the contract this patch
  realizes. **Proposed to fold together with this patch.**
- **`[ESC-MKT-SLUG-RESOLVE]`:** resolved by this pair — registry pointer to be updated at fold
  time.
- **`W3-MKT-1`:** the M2 pilot-slice WP card builds against this contract once folded.

---

*End of Doc-5D_VendorSlugResolve_Additive_Patch_PROPOSAL — realizes
`marketplace.resolve_vendor_slug.v1` on `GET /marketplace/vendor_slug_resolutions/{slug}` with the
uniform not-found collapse covering all three non-visible causes, including the two-hop
migrated-then-hidden case (V-1/V-2); no new POLICY key; adopts
`marketplace.public_read_rate_limit`. Linked-pair with
Doc-4D_VendorSlugResolve_Additive_Patch_PROPOSAL — proposed to be approved and folded together.
Awaiting owner/Board ruling.*
